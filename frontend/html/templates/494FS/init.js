...
var Presets = function(data, unit) {
  ...
  this.table = function () {
    ...
    if (ac == 'AH-64D') {
      ...
      var col_group = '<col width=40px /><col width=100% /><col width=18px/>'.repeat(priorities.length-1)

      var headers = `<td class="bg-blank" style="border:0"></td>`;
      for (const idx in priorities) {
        headers += `<th colspan=2>${priorities[idx]}</th>`;
        ...
        <table class="kb-width std std-striped" style="table-layout: fixed">
          ...
          <tbody>
            <tr class="header">
              ...
              <th colspan=2>${priorities[start]}</th>
              <td class="bg-blank" style="border:0"></td>
              <th colspan=2>${priorities[start+1]}</th>
              <td class="bg-blank" style="border:0"></td>
            </tr>`;
